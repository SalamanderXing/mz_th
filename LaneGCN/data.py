...
import os
import copy
from forecasting_data_loader import ArgoverseForecastingLoader
from av2.map.map_api import ArgoverseStaticMap as ArgoverseMap
...
class ArgoDataset(Dataset):
    def __init__(self, split, config, train=True):
        ...
        if "preprocess" in config and config["preprocess"]:
            ...
            self.am = ArgoverseMap.from_map_dir("./")
        ...
        for lane_id in lane_ids:
            ...
            if x.max() < x_min or x.min() > x_max or y.max() < y_min or y.min() > y_max:
                ...
                lane.polygon = np.matmul(
                    data["rot"], (polygon[:, :2] - data["orig"].reshape(-1, 2)).T
                ).T
                lanes[lane_id] = lane
...
class MapQuery(object):
    ...
    def __init__(self, scale, autoclip=True):
        ...
        self.map = dict(MIA=mia_map, PIT=pit_map)
        self.OFFSET = dict(
            MIA=np.array([502, -545]),
            PIT=np.array([-642, 211]),
        )
        self.SHAPE = dict(MIA=(3674, 1482), PIT=(3043, 4259))
    ...
    def query(self, region, theta=0, city="MIA"):
        ...
        rot_map = rotate(
            results, theta, center=None, order=0
        )  # center None->map center
        H, W = results.shape
